...
`nina_image_prefs.revision` is a **different mechanism** on the image-generation surface — bumped in
SQL by its own upsert, read by the photo pipeline — and this set deliberately did not touch it;
every row, gotcha and column-table entry above that speaks of it stays true.
