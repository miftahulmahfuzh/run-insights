...
- **Chat UI logic** — the pure, node-testable decisions the chat screen makes (grouping, reveal
  timing, the two idempotent appends that fill the list, scroll restore, reply quotes, and the
  full-screen chrome's geometry — where the composer and the two floating controls sit, as CSS
  lengths), kept out of the components so they can be tested.
...
below),
`perceptual.ts` (since the media-dedupe follow-up — the perceptual twin gate, the write-time half
of the sweep's perceptual pass over the `perceptual_hash`/`perceptual_sig` columns
`perceptualSign.ts` fills; **zero imports** by the same contract, and since `P1-NIN-A035` a
TWO-path predicate — see *"The perceptual twin gate answers for two resolutions now"* below),
...
pure tracking-screen vocabulary and, since `P1-RI-A033`, `planJobPhoto` — the Detail-foto row's
photograph rule), `jobActions.ts` (the `'use server'` mutations `/nina/jobs`'s rows
call).
...
`album.ts` *(T)* — the album/gallery mappers and, since `P1-RI-A032`, the whole `/nina/about`
viewer's `?photo=` codec beside them (see *"The `/nina/about` viewer's `?photo=` codec lives here
now"* below); since `P1-RI-A033` its `aboutPhotoHref` has the codec's first outbound writer,
`jobview.ts`'s `planJobPhoto`; `albumActions.ts`, `attach.ts` *(T)*.

### Chat UI logic (pure, node-testable)
`chatview.ts` *(T)*, `reply.ts` *(T)*, `reveal.ts` *(T)*, `scroll.ts` *(T)*, `live.ts` *(T)*
(`mergeServerMessages` — the refresh's server-order/local-content merge — and, since
`P1-NIN-A034`, `appendNewBubbles`, the reveal's id-idempotent half of the same list; see *"Two
deliveries, one list, both idempotent"* below), `edit.ts` *(T)* (the edit/delete rules for one
message, all three bubble gestures —
see *"Tapping a bubble opens the actions sheet"* below — **and** `canResendMessage`, R5's
his-bubbles-only gate for the sheet's third item), `chrome.ts` *(T)* — plus `turnflight.ts` *(T)*,
the flight numbers the chat screen and the poll must not restate: the poll's backoff schedule, the
give-up and the `ninaFlightView` `awaiting`/`cursor` view shared by `app/nina/page.tsx`,
`ChatScreen` and `pollNinaReply`. **Zero imports** — no database, no env reader, no `server-only` —
so all three runtimes read one copy of every constant (since `P1-RI-A038` the cold-load view carries
the live-claim disjunct too; see *"The chat turn is asynchronous"* below).
...
**The push seam is deliberately not used for this.** `lib/nina/live.ts` still serves proactive pushes
(untouched by that set; since `P1-NIN-A034` it also hosts the reveal's own append — see *"Two
deliveries, one list, both idempotent"* below). Two independent reasons, both in `pollNinaReply`'s
header: the platform requires a
...
### The cold load reads the claim, and the give-up is the background budget (`P1-RI-A038`, phase 1 of 2 of `NINA_OFFLINE_REPLY_PLAN.md`)

Two measured gaps, both about a turn that outlives a 90-second window. A runner who sent a message,
locked his phone and came back at t ∈ (90 s, 240 s) — inside a chained burst's honest run — reopened
to a quiet screen whose `awaiting` came from the message window alone, started no poll, and let her
replies land unobserved until a refresh (gap G1). An OPEN tab waiting on the same burst hit
`NINA_TURN_POLL_GIVE_UP_MS` at 90 s, raised the 'no-reply' notice over a turn that was alive, and
...
from `chatturn.ts`, the same indexed read `pollNinaReply` makes — as the sixth element of the page's
`Promise.all` (`Promise.resolve(null)` when there is no active session, on the `?photo=` branch's
idiom, so a runner with no conversation pays nothing) and hands `pendingTurn?.createdAt ?? null` to
`ninaFlightView(rows, Date.now(), liveClaimCreatedAt)`. The view ORs exactly what the poll ORs: a
FRESH claim — `nowMs - createdAt < NINA_TURN_STALE_MS`, exclusive at the boundary like the window —
OR `ninaAwaitingByMessage`'s newest-row-is-his-and-younger window, which survives as the other
disjunct because it covers the half-second hand-off between two chained turns, where one claim has
closed and the next has not yet opened. One definition of "unanswered" for the cold load and the
poll.

**The freshness comparison is load-bearing, because `getPendingNinaChatTurn` deliberately hands back
an EXPIRED pending row too.** Trusting the row's existence would re-create G1's mirror — a screen
stuck awaiting on a turn the sweep already closed. The page stays read-only: an expired claim is
passed through and declined, never swept (a sweep is a write, and this render remains indexed reads
plus `after()`). Which also makes the quiet screen honest: with no fresh claim AND an old message
there is nothing in flight, and the dead turn is phase 2's revive, not this view's business. The
third parameter defaults to `null`, so every two-argument call — the suite's whole pre-phase surface
— behaves exactly as before.

**The give-up moves from the stale deadline to `NINA_BACKGROUND_BUDGET_MS`.** What stops a DEAD
turn's poll was never the backstop: the sweep closes the claim inside `NINA_TURN_STALE_MS`, and the
next poll's claim read answers the authoritative `awaiting: false` — so the backstop's one
irreplaceable job is the poll that cannot reach the server at all (`ok: false` forever, an offline
phone), and THAT number must span the longest honest run rather than the longest dead delay: first
turn plus `NINA_TURN_CHAIN_MAX` (2) follow-up links at ~50–70 s each is ~150–210 s, inside 240 s.
Raising it stretches no outage — a failed poll is a "try again", and the server's `awaiting: false`
still ends every wait it can reach.

**Asserted where the numbers live.** `lib/nina/turnflight.test.ts` rewrites the give-up case as the
pairing — `NINA_TURN_POLL_GIVE_UP_MS` to be `NINA_BACKGROUND_BUDGET_MS` — so the old identity cannot
quietly return, and adds *"the cold load and the poll agree on 'unanswered'"*, which transcribes the
poll's disjunct (a pure unit test cannot import a `'use server'` module; if the poll's disjunct
changes, the transcription changes in the same commit) and walks both halves through the same five
cases: the G1 reopen, the dead turn's honest quiet, the expired claim neither side trusts, the window
answering alone with no claim, and the persisting tail where a fresh claim outranks her newest row.
`ninaFlightView`'s block gains the fresh-claim flip with the cursor unmoved, the expired-claim
refusal, and the default pinning the two-argument calls. The prose the change made false went with
it: `ChatScreen`'s flight-prop docstring no longer argues "zero extra queries" — the page's
...
## Two deliveries, one list, both idempotent (`P1-NIN-A034`, phase 1 of 1 of the `NINA_DUP_BUBBLE_REVEAL_PLAN.md` set)

Prod chat "gj" (2026-09-11) rendered **seven bubbles from the four `nina_messages` rows the database
holds**. The mechanism was an interleaving the refresh design had already sanctioned: `revealBubbles`
(`components/nina/ChatScreen.tsx`) appends the poll's bubbles one `sleep` apart, and a full-route RSC
delivery of `/nina` landed mid-reveal — its `read_at` sits 14 ms after the turn's INSERT, so its
payload carried all four committed rows. `mergeServerMessages`, correctly by its own rule, delivered
the three bubbles the reveal had not reached yet, and the reveal then appended its own copies of the
same ids: **two channels into one list, and only one of them deduped.**

The fix lives where the second writer lives: **`appendNewBubbles(current, bubbles, dayISO)` in
`live.ts`**, the append-decision `revealBubbles` runs inside its `setMessages` updater. It is
id-idempotent — ids the list already holds are skipped and the rest append in order, as the exact
`RevealRow` the inline literal in `revealBubbles` used to build — and it returns the **same array
reference** when nothing is new (an empty batch, or a batch the merge has already delivered), so the
state update becomes a bail-out for React, the same one `mergeServerMessages` gives the refresh, on
the other channel. The seen-set joins each batch id AS IT PASSES rather than afterwards, so a batch
repeating one id appends it once — `listNinaMessagesAfter` cannot produce that today, but the
promise is about ids, not about one caller's current impossibility.

**Why the check runs INSIDE the updater.** The reveal appends from inside a state updater because
what it appends must be decided against the list AS REACT WILL COMMIT IT — a merge can land in any
gap between two sleeps, so a check against the list the caller last saw answers a stale question.
Inside the updater it cannot be stale: the merge that lands between two sleeps is already on screen
for the next iteration.

**`live.ts` imports neither shape it decides about.** `PollBubble` is `SentBubble`
(`lib/nina/actions.ts`, `'use server'`) and `RevealRow` is the nina half of `ChatMessage`
(`components/nina/types.ts`), both restated structurally for the same reason `LiveMessage` is, and
held to the real things by `tsc` at the call site instead — the caller passes `SentBubble[]` and
assigns the result to `ChatMessage[]`, so widening either real type with a required field fails to
compile rather than silently dropping that field from her new bubbles.

**What deliberately did not change.** The merge's contract is untouched — server order, local
content, and the one-frame delivery that set all this up remains the SANCTIONED behaviour for a
refresh, including mid-reveal. The stagger is untouched too: the sleeps still run for the batch as
the poll received it, because pruning the plan for ids already on screen would need a synchronous
read of `messages` that the updater-only shape deliberately refuses as a second source of truth. So
a mid-reveal merge can still collapse the remaining stagger into one frame and leave a gap where a
bubble already sits — cosmetic, and accepted (invariant 4 of the plan). What it can no longer do is
render an id twice.

**Why the guard lives in the append and not in the merge.** Because the merge is already correct,
and making it reveal-aware — routing new nina rows through the stagger — would put a second writer
on the reveal's rhythm to fix a defect that is one missing `id` check wide.

**Tested in `live.test.ts`'s `appendNewBubbles` block** over structural rows, for the same reason the
merge's tests use them: the happy poll path, the same-reference bail-out on an empty batch, the
same-reference bail-out when the merge already delivered every id (the merge-won case), the measured
"gj" interleaving itself — `b1` on screen, batch `[b1..b4]`, exactly `[b2, b3, b4]` left to append
and the reference CHANGED, so the bail-out is pinned to the nothing-new cases rather than as a
blanket identity — the exact row shape the reveal has always appended, nothing more and nothing
less, and one id repeated inside the batch appended once.
...
any `server-only` module — which is what lets three client components and a bare node suite load it
alike; its one package-internal value import, `aboutPhotoHref` from `album.ts`, is the about-grammar
builder and is as pure as this file is (**`npm run build` is the only gate that enforces that**,
since no guard script inspects imports).
...
never enters that region. (True of R3's own diff; the codec has since moved out of the screen and
the deep link has since learned to resolve outside the window — next section.)

## The `/nina/about` viewer's `?photo=` codec lives here now, and an any-age deep link resolves on the server (`P1-RI-A032`, phase 1 of 2 of `JOB_PHOTO_LINK_PLAN.md`)

The codec `NinaAboutScreen.tsx` kept private since R3 — `encodePhoto` / `decodePhoto` over
`?photo=album.<id>|chat.<id>` — is lifted into this package's `album.ts`, because it has **two
parsers now**: the screen derives the viewer's open state from the URL on every render, and the page
(`app/nina/about/page.tsx`) has to resolve a `chat.<id>` the gallery window missed **before it can
render**. One grammar, one home — the argument `JOB_JUMP_PARAM` makes for itself in `jobview.ts`.
The shipped names are `NINA_ABOUT_HREF` (`'/nina/about'`), `NINA_ABOUT_PHOTO_PARAM` (`'photo'`),
`type NinaViewerSection = 'album' | 'chat'`, `encodeAboutPhoto` / `decodeAboutPhoto`, the
`aboutPhotoHref(section, id)` builder, `NinaAboutViewerLists` / `aboutViewerLists`, and
`aboutPhotoIdOutsideGallery(raw, gallery)`.

**There are two `?photo=` grammars and they are not each other.** `attach.ts`'s
`/nina?photo=avatar:<id>|image:<id>` is the composer-attach pointer on a different route and stays
byte-identical. The two share the KEY — `ChatScreen` deletes both in one `replaceState` for exactly
that reason — but their VALUE grammars differ (`section.id` with a dot here, `kind:id` with a colon
there), each module owns its whole round trip, and that is why this module's constant is
`NINA_ABOUT_PHOTO_PARAM` and not a second `PHOTO_PARAM`: a file importing both must never be left to
guess which grammar a bare `'photo'` spells. The dot itself is load-bearing — `URLSearchParams`
leaves `.` unencoded.

**`album.ts` is still pure, and its imports are now counted instead of denied.** `./crop` is a TYPE
so it erases; the one new value import is `@/lib/id`'s `isValidId`, one regex predicate with no
dependency behind it — the same file `jobview.ts` reaches for when a URL-borne id must be
shape-checked before it is allowed to become a query. Everything the codec exports is node-testable,
which is where its suite lives (`tests/nina.aboutPhoto.test.ts`).

**The page resolves the miss, on the server, and `description` stays there.**
`PageProps<'/nina/about'>` is Next 16's globally available helper — not an import — and
`searchParams` is a PROMISE that must be awaited. `aboutPhotoIdOutsideGallery(photoParam, gallery)`
runs over rows already in hand — pure: parse, section, shape, membership, no query — so the common
page view (no parameter, or one the newest `NINA_GALLERY_LIMIT` holds) costs exactly the three
indexed reads it always cost. Only a MISS reaches `getNinaMessageImage`, the existing single-row
deep-link read: owner-scoped to `(user_id, id)`, and it never filters `isOriginalPhoto()`, so an
album face re-attached into the conversation re-opens too. A row that resolves is mapped through
`galleryPhotos([row])[0] ?? null` ON THE SERVER, and that mapping is the invariant, not a nicety:
the read projects `imageColumns`, so the row carries `description` — `glm-4.6v`'s private prose
(invariant 5) — and `galleryPhotos` is the step that strips it. A deleted or foreign id arrives as
`null` and behaves exactly like the old `index < 0`: a closed viewer, never an error.

**The viewer renders over the merged list; the grid keeps mapping `gallery`.**
`aboutViewerLists({ album, gallery, resolvedChatPhoto })` is the one decision the URL is allowed to
change: the chat arm is `gallery` plus the resolved photo **appended at the end**, which is
load-bearing twice — the Media grid maps the `gallery` prop itself, so cell `i` and viewer index `i`
address the same photograph for every `i < gallery.length` (plan invariant 8: a deep link never
changes what the grid shows), and a resolved out-of-window photograph is by definition older than
everything in the window, so last is where newest-first order says it belongs. The screen reads
every viewer photo — `open`, `openAt`, `onIndex`, `attach` and `openChatPhoto` — out of THAT object,
so the delete control cannot silently vanish for a resolved photo. The album arm never carries the
resolved photo: an `album.<id>` deep link has no resolver behind it on purpose — nothing outside a
tap on the album grid mints one, the album read is unpaginated, and an avatar id is not a
`nina_message_images` row; `aboutPhotoIdOutsideGallery` refuses the `album` section while
`decodeAboutPhoto` keeps parsing it, because the screen opens album photos through that codec.

`aboutPhotoHref(section, id)` builds `/nina/about?photo=chat.<id>` and is the sanctioned builder for
that grammar; the screen's own taps deliberately do NOT go through it (`urlWithPhoto` sets the key
on `window.location` because it must preserve whatever else is on the current URL, while a
`<Link href>` has nothing to preserve). Phase 2 of this set (`P1-RI-A033`) is the consumer it was
built for: `planJobPhoto` in `jobview.ts` is the codec's first OUTBOUND writer, and the
Detail-foto row is its territory — next section.
...
`planJobJump`, for the same `environment: 'node'` reason — and the read is `queries.ts`'s **§12**.

### `getNinaJobPhoto` — the only job→photo key the schema has

`nina_message_images` carries no job id. Both writers of a finished selfie spell the chain the same
way — `scripts/nina-image-worker.ts`'s raw-SQL `finishSelfie` and the in-platform twin
`imagerun.ts` insert one `nina_messages` carrier with `turn_id = jobId`, then one
`kind='generated'` image row hanging off it — so the `innerJoin` on the carrier message is not one
way to answer the question, it is the only one (`listNinaSelfieJobIdsSince`, §11, walks this exact
join in the other direction; this is that read with a point instead of a list). Every property is
load-bearing:

- **Owner scope is spelled on BOTH tables.** The images predicate is the module's standing rule; the
  messages predicate is spelled too, even though the page has already owner-verified the job through
  `getNinaImageJobDetail` — a join's WHERE is where this module proves ownership, and a job id is a
  claim wherever it arrives from. The redundancy costs one predicate, not one round trip.
- **`kind = 'generated'`** excludes HIS uploads, which share the table; a job's photograph is always
  hers.
- **The order is the gallery's own** — `(created_at desc, id desc)`, `listNinaMessageImages`' — so
  `LIMIT 1` is deterministic even if a job ever carried two photographs; today both writers write
  exactly one, so the tiebreak is insurance rather than a fix.
- **The projection is `{ id }` and not `imageColumns`.** The page maps the row to an href and
  nothing else, so `description`'s exclusion (invariant 5) is STRUCTURAL — `glm-4.6v`'s private
  prose is never selected, not merely unrendered. Widen it only with a consumer.
...
returns keeps naming the photograph after a Replace and the viewer it opens shows the new bytes
(R4) — which is why the link names the row id and nothing derived from the bytes.
`removeChatPhotoAction` deletes the row outright (R3's stated boundary), the read returns `null`,
and the page renders NO control — never a link the server has not proved. The `null` is genuinely
ambiguous by design: a removed session cascades the carrier message away, the `innerJoin` misses,
and the photograph — orphaned, `message_id SET NULL` — stays alive in Media but unreachable from
Detail foto. That degradation is DECIDED (repairing it needs a `job_id` column, a migration this set
forbids), so the two nulls — "not yours" and "does not resolve" — are one outcome, this module's
standing rule.
...
separate, and this module names the fact while the codec owns the spelling (parameter name, dot,
route) — the same division `ninaJumpHref` keeps with `SESSION_PARAM`. **It is the codec's first
outbound writer, which is why the codec had to move to `lib/` at all.**

### The page reads sequentially, and skips an avatar job outright
...
This read makes a photograph RENDER — the Detail-foto icon is drawn from the row it returns — which
...
the photo control is SQUARE at the row's end — the attach strip's delete is the precedent for
sitting beside rather than filling. Both controls are `variant="secondary"` (every icon-only control
in the app is a tinted disc; an icon-only `primary` would be a black square whose meaning the reader
guesses), and the glyphs are inlined Lucide verbatim — `message-circle` for the conversation that
...
## The perceptual twin gate answers for two resolutions now (`P1-NIN-A035`, phase 1 of 1 of the `FIX_CROSS_RESOLUTION_PERCEPTUAL_DEDUPE_PLAN.md` set)

`content_hash` answers "these exact bytes are already stored", and a photograph that traveled
download → save → re-upload fails that question BY DESIGN — the journey re-encodes the bytes. The
perceptual pass sees through a re-encode by comparing SIGNATURES instead: the 64-bit dHash and the
256-byte 16x16 grayscale thumbnail that `lib/nina/perceptualSign.ts` measures and
`nina_message_images.perceptual_hash` / `.perceptual_sig` store. The gates over those signatures
live in `perceptual.ts` — **zero imports**, the `imageDedupe.ts` contract, the gate being arithmetic
over ints, bigints and byte arrays, which is what lets `tests/nina.perceptual.test.ts` assert it
with no database, no sharp and no mock.

### Two paths, and a pair must clear EVERY gate on its path

**Same dimensions** (`width === width && height === height`) — the recode case the layer was built
for (`bjNniaR6_0dY` + `IGwGhWzPNmaR`, both 736x981, dHash 0/64, mean-abs 0.043): dHash distance ≤
`PERCEPTUAL_MAX_DHASH` (1) and 16x16 mean-abs ≤ `PERCEPTUAL_MAX_SIG16` (2). `P1-NIN-A035` left this
path byte-for-byte unchanged, which is what makes the extension zero-risk to the case already
working in production.

**Cross-resolution** (dimensions differ) — 2026-09-11's measured defect: the SAME photograph
reaching the collection twice through two paths that each resize to their OWN target.
`QbZH2v65ZeKE` (714x1270, upload) and `VW04cyH9omoX` (576x1024, generated): aspect ratios 0.5622
vs 0.5625, dHash 2/64, mean-abs 1.52/255 — and the old exact-width/height gate rejected the pair
before ever reading how close the hashes were, which is why it survived every dedup layer. The
path requires ALL of:

1. relative aspect-ratio difference ≤ `PERCEPTUAL_ASPECT_TOLERANCE` (0.01) — the precondition the
   hash comparison actually needs, because `fit:'fill'` onto a FIXED grid makes a signature
   resolution-independent by construction but NOT ratio-independent;
2. BOTH per-dimension size ratios ≥ `PERCEPTUAL_MIN_SIZE_RATIO` (0.5) — a thumbnail is not a twin
   of the photograph it was cut from, however identical the pixels; the guard is inclusive at
   exactly 0.5;
3. dHash distance ≤ `PERCEPTUAL_CROSS_RES_MAX_DHASH` (3) — the cross-res path's OWN ceiling, because
   two independent resize passes carry more resampling noise than a recode at one size (measured
   2/64, pinned one step above, the same methodology that set `PERCEPTUAL_MAX_DHASH` itself:
   measured 0 → pinned 1);
4. 16x16 mean-abs ≤ `PERCEPTUAL_MAX_SIG16` — deliberately the SAME ceiling both paths use: it
   already clears the measured cross-res value and is the stronger signal of the two, so it has no
   cross-res variant.

The change is strictly additive: no pair that was `true` becomes `false`, and the same-dimensions
path never feels the relaxation — the exact vector that passes at 714x1270-vs-576x1024 is refused
when both sides are 576x1024, because the ceilings never mix.

### The gates are the sweep's, verbatim, in TWO files

`scripts/nina-dedupe-plan.mjs` (the sweep's pure half) mirrors the predicate and all five constants
verbatim — the sweep is `.mjs` and cannot import this package, so the two copies stand next to each
other with the rule between them: **if one number moves, move BOTH**, and re-read the sweep's header
first. Every gate was measured on a real production pair and pinned one step above the measurement,
because a perceptual merge can destroy a near-miss (two shots of the same court are not duplicates).
A write-time twin that only the sweep's gates would reject is a row the sweep would refuse to merge;
the two answers must not disagree.

### What the change reached, and what it deliberately did not

The write-time caller (`actions.ts`'s STEP 1b twin check over `findNinaSignedOriginals`) needed no
edit — it already hands the gate every signed original with no dimension pre-filter in SQL — and the
sweep's clustering, `electKeeper` and op shapes were already dimension-agnostic, so a cross-res
group merges through the same `merge-row` + `release-blob` ops. That production step ran: the dry
run first (the gate — both ids named in one `PERCEPTUAL FINDING`, no `--apply`), then the apply,
then verification from the DATABASE rather than the script's own summary — exactly one non-null
`source_image_id` naming the other row, whose `blob_url`/`width`/`height`/`bytes` are the keeper's,
and `/nina/about` showing the photograph once.

Left alone deliberately: `perceptualVerifyCandidates` (the sweep's stale-signature repair pass)
still shortlists candidates by `(user, width, height)`, so it cannot surface a cross-resolution
pair whose STORED signature is stale. Neither production row involved carried a stale signature,
so the pass's logic is untouched — only its comment changed, to stop asserting the narrower world —
and aspect-ratio bucketing there is separate follow-up work.
...
7. The client renders with `reveal.ts` timing, `chatview.ts` grouping, `reply.ts` quotes,
   `live.ts` merging the refresh and appending the reveal's bubbles id-idempotently, `scroll.ts`
   restore.
...
- **The five `PERCEPTUAL_*` gates live in two files and only ever move together.**
  `lib/nina/perceptual.ts` and `scripts/nina-dedupe-plan.mjs` each carry `PERCEPTUAL_MAX_DHASH`,
  `PERCEPTUAL_MAX_SIG16`, `PERCEPTUAL_ASPECT_TOLERANCE`, `PERCEPTUAL_MIN_SIZE_RATIO` and
  `PERCEPTUAL_CROSS_RES_MAX_DHASH`, because the sweep is `.mjs` and cannot import the package. If
  one number moves, move BOTH — a write-time twin that only the sweep's gates would reject is a row
  the sweep would refuse to merge. Every gate was measured on a real production pair and pinned one
  step above the measurement, and both suites pin the values as known-answer vectors, so loosening
  one "to see what happens" is exactly how a near-miss (two shots of the same court) gets destroyed.
...
**The about codec is tested where it now lives, and its wiring as source claims**
(`tests/nina.aboutPhoto.test.ts`, six describe blocks, `P1-RI-A032`). The pure half is covered
directly: the round trip for both sections and the refusal of every shape that is not one section
and one id — `null`, `undefined`, the repeated-parameter array arm, the empty string, no dot, a
leading dot, an empty id, a section the viewer does not have — with the dot-not-colon spelling
pinned against the attach grammar (the encoded value must contain no colon); `aboutPhotoHref` naming the about
route and the photo parameter and NOTHING else, and surviving both the platform's own URL parse and
the screen's decode; `aboutViewerLists`' append rule — nothing resolved means the chat arm IS the
gallery, a resolved photo lands at index `gallery.length` so every grid index still addresses the
same photograph, and the album arm never carries it; and `aboutPhotoIdOutsideGallery` answering the
one id the server must resolve and `null` for everything else, including a well-formed id
`isValidId` cannot accept. `environment: 'node'` cannot render either consumer, so the wiring is
asserted the way the `replaceState` count is — as source claims whose wording rots loudly:
`app/nina/about/page.tsx` must await the `PageProps<'/nina/about'>` promise, gate the single-row
read on the membership miss, and cross the row only through `galleryPhotos` (a claim that also
asserts the word `description` appears NOWHERE in the stripped code); `NinaAboutScreen.tsx` must
derive every viewer list through `aboutViewerLists`, read the open photo by index out of the list
the viewer shows, keep the grid on `cells={gallery.map(toCell)}`, and import nothing from the attach
module.

**The photograph link is tested at the same two altitudes** (`P1-RI-A033`).
`tests/nina.jobview.test.ts` covers the pure half: `planJobPhoto` names the row id in the about
grammar ON the about route — the href parses to `/nina/about` with `photo=chat.<id>` and contains no
colon, the attach grammar's spelling — answers `{ kind: 'none' }` for a job whose row is gone
(admin Remove and a removed session cascade are the same `null`), and never links an avatar job even
when a row id somehow arrived, because the rule lives in the arm and not only in the page's query
skip. `tests/nina.photoRefs.test.ts` carries the read half twice: a SQL-level describe pins the join
(`inner join` on the carrier message, `user_id` spelled on BOTH tables, `turn_id` and `kind` in the
WHERE, `params` carrying the job id), the gallery's own order behind `limit`, and the structural
projection (`select "nina_message_images"."id" from` — the word `description` appears NOWHERE in
...
**The perceptual twin gate is tested as known-answer vectors, once per copy of the predicate
(`P1-NIN-A035`).** `tests/nina.perceptual.test.ts` splits `isPerceptualTwin` into two describes:
the retitled *"the same-dimensions path, unchanged"* (the literals `PERCEPTUAL_MAX_DHASH = 1` /
`PERCEPTUAL_MAX_SIG16 = 2` asserted, and the old different-dimensions cases still false with the
reason re-derived — those vectors differ in SHAPE, far beyond `PERCEPTUAL_ASPECT_TOLERANCE`, not
merely in size) and the new *"the cross-resolution path"* — the measured production pair spelled
out in constants first (dHash 2/64 as two flipped low bits, mean-abs 1.52 as 133 cells two levels
apart and 123 one level apart), pinned before it is used, then: acceptance both ways round (the
predicate is symmetric), its own ceiling inclusive at 3 and refusing at 4, the same-dimensions
refusal of the very vector that passes across resolutions, a thumbnail refused at an IDENTICAL
ratio (so only the size-ratio guard can be doing the refusing), inclusivity at exactly
`PERCEPTUAL_MIN_SIZE_RATIO`, a rotated crop refused before any hash is read, and null dimensions
refused first of all. `tests/nina.dedupeMedia.test.ts` mirrors the block over
`scripts/nina-dedupe-plan.mjs`'s copy — the same vectors, so the two predicates cannot drift apart
silently — and adds a `buildPerceptualMergePlan` case clustering the two production ids into one
`perceptual` group with exactly `['merge-row', 'release-blob']` ops, the keeper deliberately
un-pinned (both fixtures are description-less, so `electKeeper`'s final tie-break decides, and
pinning it would pin an accident). `tests/nina.chatDedupe.test.ts` re-spells one STEP 1b fixture
only: its "non-twin" row was 640x853 — the keeper's own 0.750 ratio at ~87% size — which the
widened gate correctly merges now, so the fixture moved to 640x1138, a different SHAPE (0.562),
keeping the assertion's meaning instead of its pixels.
