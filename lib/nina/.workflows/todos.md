...
- [x] **P1-NIN-A014** Phase 2: Soft delete: `nina_turns.deleted_at` and the tidy list
  ...
  - **Drift**: Bookkeeping, not code: the plan index's TaskID column held an em dash for both phases (Step 3 was skipped as a re-run, so it had never been written back). Filled it in with P1-NIN-A013 and P1-NIN-A014 in /home/miftah/run-insights/.workflows/orchestration/nina-job-redo-and-soft-delete/PLAN.md.
...
- [x] **P1-NIN-A013** Phase 1: Redo: reopen a failed job from its own args
  - **Difficulty**: NORMAL
  - **Type**: Feature
