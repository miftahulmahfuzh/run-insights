...
| 7 | Retire `nina_tuning.wardrobe` | R3 | `lib/nina`, `lib/admin`, `components/admin`, `lib/db` | 23 | 2, 4 | NORMAL | `.workflows/plan/nina-image-generation-tab/phase-7.md` | `P1-NIN-A029` | **done `b7a1233`** |
...
| **Plan defect: phase 7’s journal tag count is stated three different ways** — found by phase 7, 2026-09-08 | **Take 12.** Step 4 says it *"MUST print 9"*, the Verification section says `10`, and Precondition C says `12`. Phase 7 measured **12** on the branch and `db:generate` produced `idx 12`, so 12 is right and the other two are pre-merge leftovers from before the base moved. Rung 2: the reconciled Interface Contract and this index’s phase-7 exit criteria both say 12, and they outrank a step listing that was never renumbered. **Worth correcting in `phase-7.md` if that file is ever reused**, though the set is landing so the practical value is the record: a plan carrying three answers to one arithmetic question is the same two-answers defect this index has now been amended for four times. | 2: exit criteria + the Interface Contract |
