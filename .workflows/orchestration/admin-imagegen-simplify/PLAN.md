...
**Phases:** 3
**Status:** phase 2/3 complete — phases 1-2 landed on `worktree-admin-imagegen-simplify`; a phase is complete when its row in the Phases table is ticked ✅. The set is reviewed and merged as a whole. No migration applied yet — phase 2's `0017` is committed but NOT applied; it runs post-deploy via `npm run db:migrate` (invariant 7).
