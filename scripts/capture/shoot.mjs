...
async function stills(context, browser) {
   ...
   * chart overprinted into an unreadable smear. F20 fixed it in the app (card #18 — `kmAxisTicks`
   * thins the LABELS on a stride ladder, capped at the eleven that were already legible), so the
   * long run photographs fine now and only the editorial reason is left.
   *
   * F20's cap is eleven rather than something tidier PRECISELY so this frame did not have to be
