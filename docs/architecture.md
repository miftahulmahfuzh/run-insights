...
three resolver queries live in `lib/db/queries.ts` (F03's convention) not inline; `hrMaxTransitionAt`
returns `from: HrMax | null`; the runs list *is* `/` (decision b); `AUTH_URL=''` treated as unset.
