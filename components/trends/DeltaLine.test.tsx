...
function lineFor(delta: VolumeDelta, runCount = 4, periodNoun = 'week') {
  const { container } = render(<DeltaLine delta={delta} runCount={runCount} periodNoun={periodNoun} />)
  return container.querySelector('p')!.textContent ?? ''
}

describe('DeltaLine', () => {
  it('a pct delta reads "N runs · ↑ X% vs last <period>"', () => {
    expect(lineFor({ kind: 'pct', pct: 12, direction: 'up', currentM: 10000, previousM: 8900 })).toBe(
      '4 runs · ↑ 12% vs last week',
    )
  })
  ...
  it('a small delta keeps one decimal — "↑ 4.6%" rounds honestly instead of to a lie', () => {
    ...
    expect(lineFor({ kind: 'pct', pct: -4.56, direction: 'down', currentM: 9000, previousM: 9500 })).toBe(
      '4 runs · ↓ 4.6% vs last week',
    )
  })

  it('a large delta drops the decimal — "↓ 15%", not "↓ 15.0%"', () => {
    expect(lineFor({ kind: 'pct', pct: -15, direction: 'down', currentM: 8000, previousM: 9500 })).toBe(
      '4 runs · ↓ 15% vs last week',
    )
  })

  it('near-zero volume is "flat", not a rounded "0%" that hides two real numbers', () => {
    expect(lineFor({ kind: 'pct', pct: 0.3, direction: 'flat', currentM: 10000, previousM: 10001 }, 4, 'month')).toBe(
      '4 runs · flat vs last month',
    )
  })

  it('the period noun comes from the caller — the same line serves week and month', () => {
    expect(lineFor({ kind: 'pct', pct: 12, direction: 'up', currentM: 10000, previousM: 8900 }, 4, 'month')).toBe(
      '4 runs · ↑ 12% vs last month',
    )
  })
  ...
  it('singularises the run count — "1 run", never "1 runs"', () => {
    ...
    expect(lineFor({ kind: 'pct', pct: 5, direction: 'up', currentM: 5000, previousM: 4000 }, 1)).toContain(
      '1 run ·',
    )
  })
})
