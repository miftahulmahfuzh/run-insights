...
beforeEach(() => {
  ...
  registerMock.mockResolvedValue({ ok: true, inserted: 1, skipped: 0 })
})
...
describe('useFolderUpload — the gesture’s decisions before any byte moves', () => {
  ...
  it('plans against the destination subtree and shows the diff before uploading', async () => {
    ...
    const putGate = gate({ url: 'https://blob.example/original', pathname: 'nina/a.jpg' })
    ...
    await act(async () => {
      putGate.resolve({ url: 'https://blob.example/original', pathname: 'nina/a.jpg' })
    })
    await waitFor(() => expect(result.current.phase).toBe('finished'))
  })
...
describe('useFolderUpload — one file through the lanes', () => {
  it('walks waiting -> thumbnailing -> uploading -> registering -> done, then finishes once', async () => {
    const measureGate = gate(measureOk())
    const putGate = gate({ url: 'https://blob.example/original', pathname: 'nina/a.jpg' })
    const registerGate = gate({ ok: true, inserted: 1, skipped: 0 })
    ...
    await act(async () => {
      registerGate.resolve({ ok: true, inserted: 1, skipped: 0 })
    })
    await waitFor(() => expect(result.current.phase).toBe('finished'))
  ...
  it('a failed THUMBNAIL upload is not a failed upload: the record registers with thumb null', async () => {
    ...
    uploadMock.mockImplementation((pathname: string) =>
      String(pathname).includes('thumb') ? Promise.reject(new Error('thumb 500')) : Promise.resolve({ url: 'https://blob.example/o', pathname: 'nina/a.jpg' }),
    )
...
describe('useFolderUpload — the batch mechanics', () => {
  it('never exceeds four concurrent PUTs', async () => {
    const putGate = gate({ url: 'https://blob.example/o', pathname: 'nina/a.jpg' })
    uploadMock.mockReturnValue(putGate.promise)
    const { result } = makeHook()
    await act(async () => {
    ...
    await act(async () => {
      putGate.resolve({ url: 'https://blob.example/o', pathname: 'nina/a.jpg' })
    })
    await waitFor(() => expect(result.current.phase).toBe('finished'))
