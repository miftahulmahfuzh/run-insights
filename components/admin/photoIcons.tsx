...
/**
 * Download the photograph. Lucide's `download` (lucide-static 1.42.0, fetched verbatim from
 * unpkg 2026-09-10) — the arrow into the tray, which is the one glyph every platform's save
 * affordance already speaks. Shared with `/admin/nina`'s rail, which is why it lives here and not
 * in a per-page file.
 */
...
/** Reset the framing to the un-cropped original. Lucide's `rotate-ccw`, verbatim. */
...
/** Share link to Nina — send this photograph to her chat. Lucide's `send`, verbatim. */
...
/**
 * Photoshop: editing an existing photograph — Lucide `square-pen`, verbatim. Moved here from
 * `AdminNavLinks.tsx` (its original home) once the image-collection rail needed the same glyph for
 * its own "open in Photoshop" button — the `DownloadIcon` precedent above.
 */
...
/**
 * Set as the image-generation anchor — the reference `input_references` seeds every generation
 * with. Lucide's `anchor`, verbatim: the one glyph that already speaks the domain's own word for
 * this, rather than reusing `PersonFrameIcon` ("her profile picture", a different setting).
 */
