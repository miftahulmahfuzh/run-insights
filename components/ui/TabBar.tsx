 ...
 *
 * `lib/nina/chatview.ts` declares the same number for the composer's resting floor, `lib/` may
 * not import it from here, and `tests/tabbar.geometry.test.ts` pins the two spellings together —
 * the same device `lib/nina/edit.ts` uses for its two character caps.
 */
 ...
 * top-heavy mismatch the composer had. Dropping the stack by half the pill's distance below the
 * safe line centres it BETWEEN the bar's top line and the pill itself, and it does so exactly
 * whatever the caption's line-height turns out to be: centre-of-grid plus half the below-space is
 * the midpoint of the two edges by algebra, not by a measured constant. On a device with no inset
 * the drop is 0 and the stack stays centred in the grid, which is the right answer when there is
 * no pill to measure to.
 *
