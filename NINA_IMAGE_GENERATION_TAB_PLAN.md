...
| **Phase 7’s migration number moved to `0013`, and unlike `0011` this one MUST be regenerated** — decided by the coordinator 2026-09-08 after `nina-emoji-shortcuts` landed | **On the branch phase 7 still generates `0012` and that is correct — do not rebase.** MEASURED: `origin/main`’s journal now holds **13** entries ending `idx 12 / 0012_messy_carlie_cooper` (applied to the live database at 16:02:49.236Z, which is also the new watermark), while this branch’s journal holds 12 ending `idx 11 / 0011_natural_nico_minoru`. So phase 7’s Precondition C (journal of 12, mint `0012`) holds **on the branch** exactly as written, and `db:generate` there will hand it `0012`. Rebasing the branch to pick up `0013` directly is refused: phase 6 runs concurrently in the same worktree, and a rebase under a live dependent is the one case the migration recipe calls unsafe. **The renumber happens at Step 5, by regeneration, and it lands as `0013`.** That is the *opposite* verdict from this set’s `0011`, and the asymmetry is the whole lesson: `0011` is **applied** and carries a **hand-written** `INSERT … SELECT`, so regenerating it would re-run `CREATE TABLE` against a live table and silently discard the backfill; phase 7’s migration is **unapplied** and **pure generated DDL** (one `ALTER TABLE "nina_tuning" DROP COLUMN "wardrobe"`), so a new hash and a fresh `when` are precisely what is wanted. **Same recipe, opposite verdicts, decided by the file rather than the rule** — and still diff old against new before deleting anything. | 1: invariant 10 (generated, never hand-named, never renamed) + 6: the measured rule that a rebase is safe only while dependents are unspawned |
